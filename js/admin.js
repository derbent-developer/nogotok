...
$('#login-form').onsubmit = async e => {
  ...
  if (!token) return;
  btn.disabled = true; btn.textContent = 'Проверяем ключ…'; err.style.display = 'none';
  A.token = token;
  try{
...
/* --------------------------------------------------------------- старт */
if (A.token){
  start().catch(e => { toast(e.message, true); logout(); });
}
